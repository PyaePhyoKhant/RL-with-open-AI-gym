...
for i_episode in range(TESTING_EPISODES):
    ...
    for _ in range(1000):
        ...
        if done:
            ...
            if i_episode > 100:
                avg.append(sum(rewards_list[-100:])/100)
            break
print('Average testing reward: ', sum(rewards_list) / TESTING_EPISODES)

if FULL_DIMENSIONS:
    with open('dimensions_full.txt', 'w') as f:
        for item in avg:
            f.write("%s\n" % item)
else:
    with open('dimensions_cut.txt', 'w') as f:
        for item in avg:
            f.write("%s\n" % item)
