...
for i_episode in range(LEARNING_EPISODES+1):
    ...
    for _ in range(1000):
        ...
        if done:
            ...
            if i_episode > 100:
                avg.append(sum(reward_list[-100:])/100)
            break

if OPTIMIZE:
    with open('with_optimize_training.txt', 'w') as f:
        for item in avg:
            f.write("%s\n" % item)
else:
    with open('without_optimize_training.txt', 'w') as f:
        for item in avg:
            f.write("%s\n" % item)
...
avg = []
# +1 so that average reward for last 100 episodes is shown
for i_episode in range(TESTING_EPISODES+1):
    if i_episode % 100 == 0 and i_episode != 0:
        print(str(i_episode) + '/' + str(TESTING_EPISODES) + ' testing episodes complete')
        print('current avg reward:', sum(last_100_reward) / (len(last_100_reward)+1))
        last_100_reward = []
    observation = env.reset()
    total_reward = 0
    for _ in range(1000):
        ...
        if done:
            ...
            if i_episode > 100:
                avg.append(sum(reward_list[-100:])/100)
            break
print('Average testing reward: ', sum(reward_list) / TESTING_EPISODES)

if OPTIMIZE:
    with open('with_optimize_testing.txt', 'w') as f:
        for item in avg:
            f.write("%s\n" % item)
else:
    with open('without_optimize_testing.txt', 'w') as f:
        for item in avg:
            f.write("%s\n" % item)
